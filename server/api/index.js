...
app.post('/api/auth/register', async (req, res) => {
  ...
  res.json({ token, user: { id: userId, username, phone: phone || '', points: 0, vipLevel: '普通会员' } });
});
...
app.post('/api/auth/login', async (req, res) => {
  ...
  res.json({ token, user: { id: user[0], username: user[1], phone: user[2], points: user[4], vipLevel: user[5] } });
});
...
app.get('/api/menu', (req, res) => {
  ...
  getDb().then(db => {
    ...
    const items = result[0].values.map(row => ({
      ...
      description: row[5], image: row[6], emoji: row[7], spicy: row[8] === 1, recommended: row[9] === 1
    }));
    res.json(items);
  });
});
...
app.post('/api/reservations', (req, res) => {
  ...
  getDb().then(db => {
    db.run('INSERT INTO reservations (name, phone, date, time, guests, notes) VALUES (?, ?, ?, ?, ?, ?)', [name, phone, date, time, guests, notes || '']);
    saveDb(db);

    const result = db.exec('SELECT * FROM reservations WHERE id = last_insert_rowid()');
    const row = result[0].values[0];
    res.json({ id: row[0], name: row[1], phone: row[2], date: row[3], time: row[4], guests: row[5], notes: row[6], status: row[7], createdAt: row[8] });
  });
});
...
app.get('/api/reservations', (req, res) => {
  getDb().then(db => {
    ...
    const reservations = result[0].values.map(row => ({
      id: row[0], name: row[1], phone: row[2], date: row[3], time: row[4], guests: row[5], notes: row[6], status: row[7], createdAt: row[8]
    }));
...
app.post('/api/orders', authMiddleware, (req, res) => {
  ...
  if (!address) return res.status(400).json({ error: '请填写送餐地址' });

  const orderId = 'ORD' + Date.now().toString(36).toUpperCase();
  const total = items.reduce((sum, item) => sum + item.price * item.quantity, 0);

  getDb().then(db => {
    db.run('INSERT INTO orders (id, user_id, total, address, note) VALUES (?, ?, ?, ?, ?)', [orderId, req.user.id, total, address, note || '']);
    ...
    saveDb(db);
    res.json({ id: orderId, userId: req.user.id, items, total, status: 'pending', address, note: note || '', createdAt: new Date().toISOString() });
  });
});
...
app.get('/api/orders', authMiddleware, (req, res) => {
  getDb().then(db => {
    ...
    const orders = ordersResult[0].values.map(row => {
      ...
      const items = itemsResult.length > 0 ? itemsResult[0].values.map(i => ({ name: i[0], price: i[1], quantity: i[2] })) : [];

      return { id: row[0], userId: row[1], total: row[2], status: row[3], address: row[4], note: row[5], createdAt: row[6], items };
    });
...
app.post('/api/admin/menu', authMiddleware, (req, res) => {
  ...
  if (!name || !category || !price) return res.status(400).json({ error: '菜品名称、分类和价格不能为空' });

  getDb().then(db => {
    db.run('INSERT INTO menu_items (name, name_en, category, price, description, image, emoji, spicy, recommended) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    const row = result[0].values[0];
    res.json({ id: row[0], name: row[1], nameEn: row[2], category: row[3], price: row[4], description: row[5], image: row[6], emoji: row[7], spicy: row[8] === 1, recommended: row[9] === 1 });
  });
});
...
app.put('/api/admin/menu/:id', authMiddleware, (req, res) => {
  ...
  getDb().then(db => {
    ...
    if (result.length === 0 || result[0].values.length === 0) return res.status(404).json({ error: '菜品不存在' });
    const row = result[0].values[0];
    res.json({ id: row[0], name: row[1], nameEn: row[2], category: row[3], price: row[4], description: row[5], image: row[6], emoji: row[7], spicy: row[8] === 1, recommended: row[9] === 1 });
  });
});
